...
export const Login = ({ className, ...props }: React.ComponentProps<"div">) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await invoke("login", { username: email, password: password });
      console.log("Login Success: ", response);
      // Wait to verify successful token creation/match from Tauri rust backend
      navigate("/main");
    } catch (error) {
            ...
            <CardContent className="grid p-0 md:grid-cols-2">
              <form className="p-6 md:p-8" onSubmit={handleLogin}>
                <FieldGroup>
                  <div className="flex flex-col items-center gap-2 text-center">
                    <h1 className="text-2xl font-bold">Заповядайте пак</h1>
                    <p className="text-muted-foreground text-balance">
                      Влезте във Вашия FocusGuard акаунт
                    </p>
                  </div>
                  {errorMsg && (
                  ...
                  <Field className="flex items-center">
                    ...
                    <Button variant="outline" type="button" className="">
                      ...
                      <span className="sr-only">Продължи с Google</span>
                    </Button>
                    {/* <Button variant="outline" type="button">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
              ...
              <div className="bg-muted relative hidden md:block">
                ...
                <div className="absolute inset-45">
                  <SpriteAnimator animationIndex={2} />
                </div>
              </div>
            </CardContent>
          </Card>
